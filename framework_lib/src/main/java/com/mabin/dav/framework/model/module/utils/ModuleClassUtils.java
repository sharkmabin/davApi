package com.mabin.dav.framework.model.module.utils;
...
public class ModuleClassUtils {
    ...
    private static final String TAG = ModuleClassUtils.class.getSimpleName();
    ...
    public static <T> List<T> getObjectsWithInterface(Context context, Class<T> clazz, String path) {
        ...
        try {
            ...
            if (objectList.size() == 0) {
                DLogger.e(TAG, "No files were found, check your configuration please!");
            }
        } catch (Exception e) {
            e.getStackTrace();
            DLogger.e(TAG, "getObjectsWithInterface error, " + e.getMessage());
        }
     ...
     * @return 对象列表
     */
    public static <T> List<T> getObjectsWithClassName(Context context, Class<T> clazz, List<String> classNameList) {
        List<T> objectList = new ArrayList<>();
        try {
    ...
    public static List<String> getFileNameByPackageName(Context context, String packageName) throws PackageManager.NameNotFoundException, IOException {
        ...
        for (String path : getSourcePaths(context)) {
            ...
            try {
                ...
                DLogger.e(TAG, "Scan map file in dex files made error. " + ignore);
            } finally {
                if (null != dexfile) {
                    try {
                        dexfile.close();
        ...
        DLogger.d(TAG, "Filter " + classNames.size() + " classes by packageName <" + packageName + ">");
        return classNames;
    }
